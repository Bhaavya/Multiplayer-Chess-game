...
public class BishopTest {
    ...
    public void testMoveToEmptySpot() {
        ...
        //move forward left
        Cell sourceCell = cells[0][5];
        Cell destinationCell = cells[3][2];
    ...
    public void testCapture() {
        ...
        board.setBoard(p1, p2);
        Cell cells[][] = board.getCells();
        Cell sourceCell= cells[6][2];
        Piece piece = sourceCell.getPieceAtCell();
        cells[1][4].removePiece();
        for(int i=5;i>2;i--) {
            piece.moveTo(board, cells[i][2], p2, piece);
            sourceCell= cells[i][2];
            piece = sourceCell.getPieceAtCell();
        }
        //capture black pawn
        Cell cellUnderAttack = cells[3][2];
        testMoveCapture(board, p1, cells[0][5], cellUnderAttack);
    }
